...
<div class="mb-8">
    <h1 class="font-display font-bold text-2xl md:text-3xl tracking-tight mb-1">Clientes Existentes</h1>
    <p class="text-slate-400 text-sm">Visualize, edite ou gerencie os clientes cadastrados na Golden Jardim.</p>
</div>
...
<div class="bg-[#121c14] border border-white/10 rounded-2xl p-6 mb-8 max-w-3xl">
    ...
    <form action="clientes.php" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="text" name="cli_Nome" required value="<?= htmlspecialchars($edit_cliente['cli_Nome']) ?>" class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#b7f052]">
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" name="cli_Telefone" value="<?= htmlspecialchars($edit_cliente['cli_Telefone']) ?>" class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#b7f052]">
        </div>
        <div>
            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">E-mail</label>
            <input type="email" name="cli_Email" value="<?= htmlspecialchars($edit_cliente['cli_Email']) ?>" class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#b7f052]">
        </div>
        <div class="md:col-span-2">
            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">Endereço</label>
            <input type="text" name="cli_Endereco" value="<?= htmlspecialchars($edit_cliente['cli_Endereco']) ?>" class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#b7f052]">
        </div>
        <div class="md:col-span-2 flex items-center gap-3 pt-2">
            <button type="submit" class="bg-[#b7f052] text-[#0f1710] font-bold py-2.5 px-6 rounded-xl hover:bg-[#9cd438] transition-all text-xs uppercase tracking-wider">Atualizar Cliente</button>
...
<div class="bg-[#121c14] border border-white/10 rounded-2xl overflow-hidden mb-8">
    <div class="p-6 border-b border-white/10 flex items-center justify-between">
        ...
        <a href="cadastrar_cliente.php" class="bg-[#b7f052] text-[#0f1710] font-bold py-2 px-4 rounded-xl hover:bg-[#9cd438] transition-all text-xs uppercase tracking-wider">+ Novo Cliente</a>
    </div>
