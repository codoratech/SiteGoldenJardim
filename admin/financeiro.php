...
if (isset($_GET['acao']) && $_GET['acao'] == 'excluir' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    mysqli_query($con, "DELETE FROM TB_ContasReceber WHERE ID_ContaReceber = $id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['id_conta']) && !empty($_POST['id_conta'])) {
        ...
        mysqli_query($con, "UPDATE TB_ContasReceber SET TB_Clientes_ID_Cliente=$cli_id, TB_OrdensServico_ID_Ordem=$os_id, Con_Valor=$valor, Con_DataVencimento='$vencimento', Con_Status='$status', Con_FormaPagamento='$forma' WHERE ID_ContaReceber=$id");
        $msg = "Conta a receber atualizada!";
    } else {
        mysqli_query($con, "INSERT INTO TB_ContasReceber (TB_Clientes_ID_Cliente, TB_OrdensServico_ID_Ordem, Con_Valor, Con_DataVencimento, Con_Status, Con_FormaPagamento) VALUES ($cli_id, $os_id, $valor, '$vencimento', '$status', '$forma')");
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-white/5 text-sm">
                ...
                <tr class="hover:bg-white/5 transition-colors">
                    ...
                    <td class="p-4"><span class="px-2.5 py-1 rounded-full text-xs bg-white/5 border border-white/10 <?= $row['Con_Status'] == 'Pago' ? 'text-emerald-400 border-emerald-500/30' : 'text-amber-400 border-amber-500/30' ?>"><?= htmlspecialchars($row['Con_Status']) ?></span></td>
