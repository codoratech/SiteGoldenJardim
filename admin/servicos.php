...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['id_servico']) && !empty($_POST['id_servico'])) {
        ...
        if (mysqli_query($con, "INSERT INTO TB_Servicos (Ser_Nome, Ser_Descricao, Ser_Preco) VALUES ('$nome', '$desc', $preco)")) {
            header("Location: servicos.php?ok=insert");
            exit();
        } else {
...
if (isset($_GET['ok'])) {
    if ($_GET['ok'] == 'insert') $msg = "Serviço cadastrado com sucesso!";
...
<div class="bg-[#121c14] border border-white/10 rounded-2xl p-6 mb-8">
    ...
    <form action="servicos.php" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
            ...
            <input type="text" name="Ser_Nome" required value="<?= (isset($edit_servico['Ser_Nome']) ? htmlspecialchars($edit_servico['Ser_Nome']) : '') ?>" class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#b7f052]">
        </div>
        <div>
        ...
        <div class="md:col-span-3">
            ...
            <input type="text" name="Ser_Descricao" value="<?= (isset($edit_servico['Ser_Descricao']) ? htmlspecialchars($edit_servico['Ser_Descricao']) : '') ?>" class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#b7f052]">
        </div>
